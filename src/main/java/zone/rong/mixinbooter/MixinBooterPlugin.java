...
@IFMLLoadingPlugin.Name("FurMixinBooter")
...
public final class MixinBooterPlugin implements IFMLLoadingPlugin {
    ...
    private void gatherPresentMods() {
        ...
        try { // newer Gsons
            GsonBuilder.class.getMethod("setLenient").invoke(gsonBuilder);
    ...
    private void recordConfigOwners() {
        for (Config config : Mixins.getConfigs()) {
            if (!config.getConfig().hasDecoration("mixinOwner")) {
                config.getConfig().decorate("mixinOwner", (Supplier) () -> this.retrieveConfigOwner(config));
            }
        }
    }
